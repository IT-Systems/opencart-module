...
class ModelPaymentSveaInvoice extends Model {
	...
	public function install() {
        ...
        $this->model_setting_setting->editSetting('svea_invoice', array('svea_invoice_use_tupas' => 1));    
        $this->model_setting_setting->editSetting('svea_invoice', array('svea_invoice_tupas_mode' => 0));
        $this->model_setting_setting->editSetting('svea_invoice', array('svea_invoice_tupas_shop_token' => $token));
        ...
        $sql = "INSERT INTO `" . DB_PREFIX . "svea_tupas` (shop_id, api_token, payment_module, previous_mode, previous_shop_token) 
                VALUES 
                    ('{$response->id}', '{$response->api_token}', 'INVOICE', 'test', '{$token}') 
    ...
    function editShopInstance() {
        ...
        // Perform a request
        $shop_info = array(
             'shop_token' => $this->request->post['svea_invoice_tupas_shop_token'],
             'mode' => $this->request->post['svea_invoice_tupas_mode'],
        ...
        if ($response->status->code === 200) {
            $sql = "UPDATE `" . DB_PREFIX . "svea_tupas` 
                        SET previous_shop_token = '{$this->request->post['svea_invoice_tupas_shop_token']}', previous_mode = '{$this->request->post['svea_invoice_tupas_mode']}' 
                    WHERE payment_module = 'INVOICE'";
            $this->db->query($sql);
        }        
    }
    ...
    function tupasSettingsChanged() {
        $sql = "SELECT previous_mode, previous_shop_token FROM `" . DB_PREFIX . "svea_tupas` WHERE payment_module = 'INVOICE'";
        $result = $this->db->query($sql);
        // Check values from post data, since config isn't updated at this point.
        return ($result->row['previous_shop_token'] != $this->request->post['svea_invoice_tupas_shop_token'] || 
                $result->row['previous_mode'] != $this->request->post['svea_invoice_tupas_mode'])
        ? true : false;
    }
    
    
    function get_previous($col='shop_token'){
        $sql = "SELECT previous_{$col} AS pval FROM `" . DB_PREFIX . "svea_tupas` WHERE payment_module = 'INVOICE'";
        $result = $this->db->query($sql);
    ...
    function getShopId() {
        $sql = "SELECT shop_id FROM `" . DB_PREFIX . "svea_tupas` WHERE payment_module = 'INVOICE'";
        $result = $this->db->query($sql);
